x = """light salmon bags contain 5 dotted olive bags, 4 wavy lavender bags.
...
y = [l.split(' contain ') for l in x.split('\n')]
z = [(l[0][:-5], l[1][:-1].split(', ')) for l in y]
zz = [(a, [(int(w[0]), f'{w[1]} {w[2]}') for w in map(lambda s: s.split(' '), b) if w[0] != 'no']) for a, b in z]
can_be_in = [(b, a) for a, bs in zz for yadda, b in bs]
tree = {}
for bag, holder in can_be_in:
    if bag in tree:
        tree[bag].add(holder)
    else:
        tree[bag] = set([holder])
...
def count(s, t):
    ...
    if not content:
        return 1
    res = 1
    for cnt, bag in content:
        res = res + cnt * count(bag, t)
    return res
